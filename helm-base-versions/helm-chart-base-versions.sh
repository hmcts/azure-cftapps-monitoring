                    ...
                    if [ $(ver "$CURRENT_VERSION") -lt $(ver "$DEPRECATED_CHART_VERSION") ]; then
                        ...
                        if [[ ! " ${NOTIFICATION_ARRAY[*]} " =~ ${CHART_NAME} ]]; then
                          NOTIFICATION_ARRAY+=("$CHART_NAME")
                            python3 send-json-to-cosmos.py --chart_name "$CHART_NAME" --namespace "$NAMESPACE" --cluster_name "$CLUSTER_NAME" --deprecated_chart_name "DEPRECATED_CHART_NAME" --current_version "$CURRENT_VERSION" --is_deprecated true --flag true                       
                          fi
                        break
                    fi
              done
