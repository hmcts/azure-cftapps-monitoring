...
log_failed_deployment_to_cosmos() {
    ...
    python3 send-json-to-cosmos.py "$cluster_name" "$namespace" "$deployment_name" "$ready_replicas" "$desired_replicas" "$slack_channel" "$slack_webhook"
}
...
if [[ $MODE == "notify" ]]; then
python3 send-notification-to-slack.py "$SLACK_WEBHOOK" 
fi